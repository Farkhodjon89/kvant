...
const ProductsListItem = ({product, catalog, slider, wishlistItems}) => {
  ...
  return (
      <div className={slider ? s.listItem2 : s.listItem}>
        <div className={s.card}>
          {product.onSale && <div className={s.discountPrice}>-{discountPrice}%</div>}
          <Link href={`/product/${product.slug}`}>
            <a>
              <Image width={350} height={380} src={product.image.sourceUrl} alt="product_img" className={s.productImg}/>
            </a>
          </Link>
          {/* <button
          ...
          <div className={s.details}>
            ...
            <div>
              {product.onSale ? (
                  <>
                    <span className={s.normalPrice}>{getFormatPrice(product.woocsRegularPrice)}</span>{' '}
                    <span className={s.salePrice}>{getFormatPrice(product.woocsSalePrice)}</span>
                  </>
              ) : (
