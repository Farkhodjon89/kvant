...
export const PaymentInput = ({payment, setPayment, paymentMethods, banks, setBank,setSelectMethod}) => {
  const bankChange = (e) => {
  ...
  return (
      paymentMethods.map((r, i) => (
          <div key={i} onClick={() => setPayment(r.value)}
            ...
            {r.name}
            {r.value === 'installment' && <select className={s.select} onChange={bankChange}>
              {banks.map((el, i) => (
                  <option>{el}</option>
              ))}
            </select>}
